...
const BASE_URL =
  globalThis.process?.env?.REHAB_API_BASE ||
  globalThis.process?.env?.VITE_API_BASE ||
  'http://localhost:8000';
...
export async function runScaleIntegrationTest() {
  console.log('\n🧪 量表系统集成测试开始\n');
  ...
  try {
    ...
    const pushResult = await request('/api/integration/scale/push', {
      method: 'POST',
    ...
    const pendingScales = await request(`/api/integration/scale/pending/${TEST_PATIENT_ID}`);
    ...
    const submitResult = await request('/api/integration/scale/submit', {
      method: 'POST',
    ...
    const pendingAfterSubmit = await request(`/api/integration/scale/pending/${TEST_PATIENT_ID}`);
    ...
    const results = await request(`/api/integration/scale/results/${TEST_SESSION_ID}`);
